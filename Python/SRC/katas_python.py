...
def media (lista_notas, nota_aprobado = 5):
    ...
    estado = "aprobado" if promedio >= nota_aprobado 
    else "suspenso"
...
def longitud(frase):
    ...
    return list(map(len, palabras))  # mide cada palabra
...
sumar3 = lambda x: x + 3
lista = [1, 2, 3]
resultado = list(map(sumar3, lista))
# resultado → [4, 5, 6]
...
def buscar_nombre():
    try:
     ...
     #  Ahora pedimos el nombre que se desea buscar
      objetivo = input("Nombre a buscar: ").strip()
        
        
        # Verificamos si está en la lista
        if objetivo in lista_nombres:
                    print(f"El nombre '{objetivo}' fue encontrado en la lista.")
        else:
...
class UsuarioBanco:
    ...
    def transferir_dinero(self, otro_usuario, cantidad):
        ...
        if cantidad > self.saldo:
            raise ValueError("No tiene saldo suficiente para transferir.")
...
def eliminar_palabra(texto, palabra_eliminar):
    ...
    palabras = texto.split()
    filtradas = [p for p in palabras if p.lower() != palabra_eliminar.lower()]
